...
def hinge_loss_full(feature_matrix, labels, theta, theta_0):
    ...
    for i in range(m):
        z_i = labels[i]*(np.dot(theta, feature_matrix[i]) + theta_0)
        if(z_i >= 1):
            loss_h[i] = 0
        else:
            loss_h[i] = 1 - z_i
            
    Loss = np.sum(loss_h)/m
...
def perceptron_single_step_update(
    ...
    if (label*(np.dot(theta, feature_vector) + theta_0) < 0) :
        ...
        theta_0 += label
    elif (abs(label*(np.dot(theta, feature_vector) + theta_0)) < eps):
        theta += label*feature_vector
        theta_0 += label
...
def average_perceptron(feature_matrix, labels, T):
    ...
    final_theta = sum_theta/(n*T)
    final_theta_0 = sum_theta_0/(n*T)    
...
def classify(feature_matrix, theta, theta_0):
    ...
    for i in range(n):
        if ((np.dot(theta, feature_matrix[i]) + theta_0) > eps):
            labels[i] = 1
...
def bag_of_words(texts):
    ...
    with open(r'stopwords.txt') as stop:
